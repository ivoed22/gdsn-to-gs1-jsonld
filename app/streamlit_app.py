...
def main() -> None:
    ...
    with st.container(border=True):
        render_workflow_entry_intro()
        # 5 workflow cards: render as 3+2 layout to avoid cramping.
        first_row_modes = WORKFLOW_MODES[:3]
        second_row_modes = WORKFLOW_MODES[3:]
        ...
        bottom_columns = st.columns([1, 1, 1])
        for column, mode in zip(bottom_columns[:2], second_row_modes, strict=True):
            with column:
                selected = st.session_state["workflow_mode"] == mode["title"]
                render_workflow_mode_card(
                    mode["title"],
